...
export default function AutoStaffAvailability() {
  ...
  const fetchStaff = useCallback(async () => {
    try {
      const res = await autoFetch("/api/users");
  ...
  const fetchSchedule = useCallback(
    async (userId: string) => {
      ...
      try {
        const res = await autoFetch(`/api/staff/availability/${userId}`);
        if (res.ok) {
          const data = await res.json();
  ...
  const fetchTimeOff = useCallback(async () => {
    ...
    try {
      const res = await autoFetch("/api/staff/time-off");
      if (res.ok) {
  ...
  const handleSaveSchedule = async () => {
    ...
    try {
      const res = await autoFetch(`/api/staff/availability/${selectedUserId}`, {
        method: "PUT",
  ...
  const handleDeleteTimeOff = async (id: number) => {
    try {
      const res = await autoFetch(`/api/staff/time-off/${id}`, { method: "DELETE" });
