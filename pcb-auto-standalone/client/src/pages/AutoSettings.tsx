...
export default function AutoSettings() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await autoFetch("/api/shop/settings", { method: "PATCH", body: JSON.stringify(payload) });
  ...
  const addBay = async () => {
    ...
    try {
      const res = await autoFetch("/api/bays", {
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await autoFetch("/api/logo/upload", {
        method: "POST",
        body: formData,
        rawBody: true,
      });
  ...
  const handleRemoveLogo = async () => {
    try {
      const res = await autoFetch("/api/shop/settings", {
        method: "PATCH",
  ...
  const deleteBay = async (id: number) => {
    try {
      await autoFetch(`/api/bays/${id}`, { method: "DELETE" });
