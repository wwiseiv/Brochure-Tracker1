...
**Parent Platform:** FSI Suite (fsisuite.ai)
...
It lives inside the FSI Suite codebase as a standalone module with its own subdomain (e.g., `shop.fsisuite.ai`), sharing the database and authentication infrastructure but presenting a completely separate merchant-facing experience.
...
| Backend Stack | Node.js / Express / TypeScript | Match FSI Suite, single codebase |
| Frontend Stack | React / TypeScript / Vite / Tailwind CSS | Match FSI Suite, dark mode UI |
| Database | PostgreSQL (shared with FSI Suite) | Single DB, tenant isolation via RLS |
| Architecture | Same app, same DB, subdomain routing | `shop.fsisuite.ai` for merchants, `app.fsisuite.ai` for FSI admin |
| First Processor | FluidPay | Hosted pages with custom branding, iframe embed, terminal support |
| Processor Dropdown | PCBancard (NMI), SignaPay (NMI/PayHub+), Priority Commerce, CardConnect/CardPointe, FluidPay | FSI admin selects at onboarding; shop never sees |
...
| Dual Pricing Config | Configurable per shop by FSI admin | Range: 3.0%–4.0%, typical 3.5–3.99% |
...
4. **ASSUMPTION:** FSI Suite's existing auth system (JWT-based) can be extended with tenant-scoped tokens that route to the PCB Auto subdomain.
...
│                    INTERNET / DNS                        │
│                                                         │
│  app.fsisuite.ai          shop.fsisuite.ai              │
│  (FSI Admin Portal)       (PCB Auto Merchant Portal)    │
│         │                         │                     │
└─────────┼─────────────────────────┼─────────────────────┘
          │                         │
          ▼                         ▼
...
| Option | Pros | Cons |
|--------|------|------|
| **A) Single DB + RLS (RECOMMENDED)** | Simple ops, shared infrastructure, easy cross-tenant reporting for FSI admin, works on Replit's single Postgres instance | Must enforce tenant_id on every query; risk of data leak if RLS misconfigured |
...
export const subdomainRouter = (req: Request, res: Response, next: NextFunction) => {
  ...
  // Default: FSI Suite admin portal
...
```
FSI Admin creates shop → generates tenant record + admin user
  → Shop owner receives email with shop.fsisuite.ai link
...
### 4.1 Shared Tables (existing FSI Suite, extended)

```sql
-- Already exists in FSI Suite; extended with PCB Auto fields
...
CREATE TABLE pcb_employees (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL REFERENCES tenants(id),
  user_id UUID REFERENCES users(id),  -- links to FSI auth system
  first_name VARCHAR(100) NOT NULL,
  last_name VARCHAR(100) NOT NULL,
...
### 6.1 PCB Auto Merchant Portal (shop.fsisuite.ai)
...
### 6.3 FSI Admin Panel Additions

Inside the existing FSI Suite admin at app.fsisuite.ai:
...
- **Launch Button:** "Open PCB Auto Portal" → opens shop.fsisuite.ai in new tab
...
|------|-----------|-------------|-----------------|
| 1 | **Foundation** | DB schema migration, subdomain routing, PCB Auto auth flow, dark mode shell with navigation, logo integration | Login at shop.fsisuite.ai renders PCB Auto dashboard shell |
...
| 6 | **Polish + Reports** | Dashboard metrics, basic sales/AR reports, audit log viewer, RBAC enforcement, FSI admin onboarding flow | Admin creates shop in FSI; shop owner logs into PCB Auto; creates RO → sends estimate → customer approves → invoice paid. Full cycle in < 10 minutes |
...
class NMIAdapter implements PaymentAdapter {
  ...
  async createPaymentLink(req: PaymentCreateRequest): Promise<PaymentLinkResponse> {
    ...
    return {
      linkUrl: `https://shop.fsisuite.ai/public/pay/${req.idempotencyKey}`,
