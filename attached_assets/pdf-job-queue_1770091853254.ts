 ...
 * 
 * INSTALLATION:
 *   Copy to: server/services/pdf-job-queue.ts
 */
 ...
 * );
 * 
 * CREATE INDEX idx_pdf_jobs_user ON pdf_parse_jobs(user_id);
 * CREATE INDEX idx_pdf_jobs_status ON pdf_parse_jobs(status);
 */

export const PDF_JOBS_SCHEMA = `
-- Drizzle schema
import { pgTable, varchar, text, jsonb, timestamp, integer } from 'drizzle-orm/pg-core';

export const pdfParseJobs = pgTable('pdf_parse_jobs', {
