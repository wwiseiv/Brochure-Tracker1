 ...
 * 
 * INSTALLATION:
 *   1. Copy this file to: server/services/statement-validator.ts
 *   2. Import and use in your statement analysis pipeline
 *   3. Update routes.ts to use the validated/sanitized output
 * 
 * USAGE:
 *   import { validateAndSanitize, StatementValidator } from './statement-validator';
 *   
 *   // After AI extraction:
 *   const rawExtraction = await extractStatementData(files);
 *   const { data, confidence, issues, needsManualReview } = validateAndSanitize(rawExtraction);
 */
...
const VALIDATION_RULES = {
  ...
  volume: {
    min: 0,
    max: 100000000,        // $100M seems like a reasonable max for SMB
    suspiciouslyLow: 100,  // Less than $100/month is suspicious
    suspiciouslyHigh: 10000000, // More than $10M/month needs review
  },
  ...
  effectiveRate: {
    min: 0,
    max: 15,               // 15% would be absurdly high
    typical: { min: 1.5, max: 4.5 },
    redFlagThreshold: 5,
  },
  
  // Card breakdown should sum to ~100% of volume
  cardBreakdownTolerance: 0.05, // 5% tolerance
  
  // Minimum confidence to auto-approve
  minConfidenceForAutoApprove: 70,
  
  // Fields that trigger manual review if missing
...
export class StatementValidator {
  ...
  /**
   * Sanitize merchant info section
   */
  ...
  private sanitizeVolumeData(raw?: ExtractedStatementData['volumeData']): SanitizedStatementData['volumeData'] {
    ...
    // Average Ticket - calculate if we have volume and transactions
    ...
    // Validate avg ticket makes sense
  ...
  private sanitizeCardBreakdown(raw: Partial<CardBreakdown>, totalVolume: number): CardBreakdown {
    ...
    // Check if breakdown sums to approximately total volume
  ...
  /**
   * Sanitize and validate effective rate
   */
  private sanitizeEffectiveRate(
    ...
    // Try to calculate from fees and volume
  ...
  private crossValidate(
    ...
    // Check volume vs transactions makes sense
  ...
  private calculateOverallConfidence(): ConfidenceScore {
    ...
    // Weight the sections (volume data is most important)
    const weights = {
      merchantInfo: 0.2,
      volumeData: 0.5,
      fees: 0.3,
  ...
  private determineManualReview(confidence: ConfidenceScore): { needsManualReview: boolean; reviewReasons: string[] } {
    ...
    // Volume section too low confidence
  ...
  private sanitizeNumber(value: any): number | null {
    ...
    // Handle string numbers (possibly with currency symbols)
