 ...
 * 
 * USAGE:
 *   1. Copy to: client/src/hooks/use-safe-statement-data.ts
 *   2. Import and use in statement-analyzer.tsx
 *   
 *   const { safeData, confidence, needsReview } = useSafeStatementData(analysisResult);
 */
...
function sanitizeVolumeData(raw: any): SafeStatementData['volumeData'] {
  ...
  // Calculate avg ticket if not provided
...
export interface UseSafeStatementDataResult {
  // Sanitized data - always safe to render
...
/**
 * Hook that provides safe access to statement analysis data
 * Guarantees all values are defined and safe to render
 */
