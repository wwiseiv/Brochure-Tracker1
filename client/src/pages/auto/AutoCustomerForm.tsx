...
export default function AutoCustomerForm() {
  ...
  const fetchCustomer = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await autoFetch(`/api/auto/customers/${params!.id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const url = isEditing ? `/api/auto/customers/${params!.id}` : "/api/auto/customers";
      ...
      setLocation(`/auto/customers/${data.id}`);
    } catch (err: any) {
