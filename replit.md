...
- **Meeting Recording for Sales Coaching** (/merchants/:id): Record meetings directly from merchant detail pages. When agents stop recording, the system automatically: (1) uploads the audio to secure storage, (2) transcribes and analyzes the actual audio content with Gemini AI (base64-encoded audio sent directly to model) to generate summaries, key takeaways, and sentiment analysis based on what was actually said, (3) emails the recording with merchant contact info to wwiseiv@icloud.com for the office to review and build an AI-powered sales coaching repository. Simple one-button interface - just tap "Record Meeting" to start and "Stop & Send to Office" when done. All past recordings are stored and accessible in a "Past Recordings" section on each merchant's page - agents can view AI summaries, key takeaways, and listen to or download any previous recording. Download endpoint (`/api/meeting-recordings/:id/download`) forces proper file download with correct headers.
- **Pickup Meeting Recording** (/drops/:id): Record pickup conversations directly from drop detail pages (for pending drops). Same flow as merchant recordings - tap to record, AI analyzes, emails to office. Perfect for capturing sales conversations during brochure pickup visits. Recordings up to 60 minutes are supported.
- **Offline Recording Retry**: If upload fails (no internet, network issues), recordings are automatically saved locally to IndexedDB. You can retry uploading indefinitely until successful. Shows online/offline status and retry count.
- **Prospect Tracking from First Drop**: Merchants are now automatically created as "prospects" when the first drop is made to a business. This means you can see the full visit history and conversation details on the Merchant record from the very first visit - not just after conversion. Status badges show Prospect/Converted/Lost status on each merchant profile.
- **Visit History on Merchant Pages**: The Merchant detail page now shows a complete "Visit History" section with all linked drops. Each visit shows the date, status, outcome, and notes. Tap any visit to see full details.
- **Auto-Convert Merchants on Signing**: When a drop is marked as "signed" (converted), the linked merchant status automatically updates to "converted" and totalConversions is incremented.
